...
public class MediaManagerPlugin: NSObject, FlutterPlugin {
    ...
    private var videoThumbnailCache = NSCache<NSString, UIImage>()
...
public class MediaManagerPlugin: NSObject, FlutterPlugin {
    ...
    private func getVideoThumbnail(path: String, result: @escaping FlutterResult) {
        DispatchQueue.global(qos: .userInitiated).async {
            if let cachedThumbnail = self.videoThumbnailCache.object(forKey: path as NSString) {
                if let thumbnailData = cachedThumbnail.jpegData(compressionQuality: 0.8) {
                    DispatchQueue.main.async {
            ...
            do {
                ...
                self.videoThumbnailCache.setObject(thumbnail, forKey: path as NSString)
    ...
    private func clearVideoThumbnailCache() {
